...
public class SE_Monk : SE_Stats
{
	...
	public override void UpdateStatusEffect(float dt)
	{
		...
		if (surging && m_SurgeTimer <= 0f)
		{
			...
			m_character.Heal(VL_Utility.GetHealingPower(EpicMMOSystem.LevelSystem.Instance.getLevel(),
				EpicMMOSystem.LevelSystem.Instance.getParameter(EpicMMOSystem.Parameter.Body),
				m_character.GetSkills().GetSkillList().FirstOrDefault((Skills.Skill x) => x.m_info == ValheimLegends.DisciplineSkillDef).m_level,
				1f) * VL_GlobalConfigs.c_monkSurge);
