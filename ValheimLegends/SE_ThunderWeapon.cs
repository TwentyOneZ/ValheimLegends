...
public class SE_ThunderWeapon : StatusEffect
{
	...
	public override void UpdateStatusEffect(float dt)
	{
		if (doOnce)
		{
			...
			m_tooltip = "Attacks are imbued with Lightning, jumps to nearby targets" +
				"\n" + ((0.1f + (level / 300f)) * 100f).ToString("#.#") + "% chance to hit for extra " + Mathf.Max((0.1f * (EpicMMOSystem.LevelSystem.Instance.getLevel() / 4f) * (1f + (level / 150f))),0.1f).ToString("#.#") + "-" + Mathf.Max((1.9f * (EpicMMOSystem.LevelSystem.Instance.getLevel() / 4f) * (1f + (level / 150f))),0.1f).ToString("#.#") + " Lightning damage that jumps for nearby targets";
		}
