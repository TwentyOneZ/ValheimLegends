...
public class Class_Mage
{
    ...
    // Buff hash (Arcane Intellect)
    private static readonly int SE_ARCANE_INTELLECT_HASH = "SE_VL_ArcaneIntellect".GetStableHashCode();
    ...
    public static void Execute_Attack(Player player)
    {
        if (QueuedAttack == MageAttackType.FlameNova)
        {
            ...
            foreach (Character item in allCharacters)
            {
                if (BaseAI.IsEnemy(player, item) &&
                    (item.transform.position - player.transform.position).magnitude <= 8f + 0.1f * level &&
                    VL_Utility.LOS_IsValid(item, player.GetCenterPoint(), player.transform.position + player.transform.up * 0.2f))
                {
                    float fireExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_fire * (1f + (level / 100f));
                    float frostExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_frost * (1f + (level / 100f));
                    float lightningExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_lightning * (1f + (level / 100f));
                    float poisonExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_poison * (1f + (level / 100f));
                    float spiritExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_spirit * (1f + (level / 100f));

                    UnityEngine.Vector3 dir = item.transform.position - player.transform.position;
                    HitData hitData = new HitData();
                    hitData.m_damage.m_fire = UnityEngine.Random.Range(5f + 2.75f * level, 10f + 3.5f * level) * VL_GlobalConfigs.g_DamageModifer * VL_GlobalConfigs.c_mageInferno + fireExpertiseDamage;
                    hitData.m_damage.m_frost = frostExpertiseDamage;
                    hitData.m_damage.m_lightning= lightningExpertiseDamage;
                    hitData.m_damage.m_poison = poisonExpertiseDamage;
                    hitData.m_damage.m_spirit = spiritExpertiseDamage;
        ...
        else if (QueuedAttack == MageAttackType.IceDagger)
        {
            ...
            UnityEngine.Vector3 target = ((!Physics.Raycast(vector, player.GetLookDir(), out hitInfo, float.PositiveInfinity, ScriptChar_Layermask) || !hitInfo.collider)
                ? (position + player.GetLookDir() * 1000f)
                : hitInfo.point);

            float fireExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_fire * (1f + (level2 / 100f));
            float frostExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_frost * (1f + (level2 / 100f));
            float lightningExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_lightning * (1f + (level2 / 100f));
            float poisonExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_poison * (1f + (level2 / 100f));
            float spiritExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_spirit * (1f + (level2 / 100f));
            ...
            hitData2.m_damage.m_frost = UnityEngine.Random.Range(0.5f * level2, 2f + 1f * level2) * VL_GlobalConfigs.g_DamageModifer * VL_GlobalConfigs.c_mageFrostDagger + frostExpertiseDamage;
            hitData2.m_damage.m_fire = fireExpertiseDamage;
            hitData2.m_damage.m_lightning = lightningExpertiseDamage;
            hitData2.m_damage.m_poison = poisonExpertiseDamage;
            hitData2.m_damage.m_spirit = spiritExpertiseDamage;
    ...
    public static void Process_Input(Player player, float altitude)
    {
        ...
        if (VL_Utility.Ability3_Input_Down && !meteorCharging)
        {
            ...
            {
                // ======= NOVO COMPORTAMENTO (BLOCKING) =======
                // Antes: recuperava Eitr baseado em Stamina.
                // Agora: toggle do buff infinito Arcane Intellect.
                if (player.IsBlocking())
                {
                    // MP-safe: só owner mexe em SE
                    ...
                    bool hasArcane = player.GetSEMan().HaveStatusEffect(SE_ARCANE_INTELLECT_HASH);

                    if (hasArcane)
                    {
                        player.GetSEMan().RemoveStatusEffect(SE_ARCANE_INTELLECT_HASH);
                        ...
                        player.Message(MessageHud.MessageType.TopLeft, "Arcane Intellect: OFF");
                    }
                    else
                    {
                        StatusEffect se = ScriptableObject.CreateInstance<SE_ArcaneIntellect>();
                        ...
                        UnityEngine.Object.Instantiate(ZNetScene.instance.GetPrefab("fx_Potion_frostresist"), player.transform.position, UnityEngine.Quaternion.identity);
                        player.Message(MessageHud.MessageType.TopLeft, "Arcane Intellect: ON");
                    }
                ...
                // ======= METEOR (SEM BLOQUEAR) CONTINUA IGUAL =======
                else
                {
        ...
        else if ((VL_Utility.Ability3_Input_Up || player.GetStamina() <= 1f || Mathf.Max(0f, altitude - player.transform.position.y) > 1f) && meteorCharging)
        {
            ...
            for (int i = 0; i < meteorCount; i++)
            {
                ...
                RaycastHit hitInfo = default(RaycastHit);
                UnityEngine.Vector3 position = player.transform.position;
                UnityEngine.Vector3 target = ((!Physics.Raycast(vector, player.GetLookDir(), out hitInfo, 1000f, ScriptChar_Layermask) || !hitInfo.collider)
                    ? (position + player.GetLookDir() * 1000f)
                    : hitInfo.point);
                ...
                HitData hitData = new HitData();
                hitData.m_damage.m_fire = UnityEngine.Random.Range(30f + 0.5f * level2, 50f * level2) * VL_GlobalConfigs.g_DamageModifer * VL_GlobalConfigs.c_mageMeteor + fireExpertiseDamage;
                ...
                GO_CastFX = UnityEngine.Object.Instantiate(ZNetScene.instance.GetPrefab("fx_guardstone_permitted_removed"),
                    player.transform.position + player.transform.right * UnityEngine.Random.Range(-1f, 1f) + player.transform.up * UnityEngine.Random.Range(0f, 1.5f),
        ...
        else if (VL_Utility.Ability2_Input_Down)
        {
            if (!player.GetSEMan().HaveStatusEffect("SE_VL_Ability2_CD".GetStableHashCode()))
            {
                ...
                else if (player.GetStamina() >= VL_Utility.GetFrostNovaCost)
                {
                    ...
                    {
                        if (BaseAI.IsEnemy(player, item) &&
                            (item.transform.position - player.transform.position).magnitude <= 10f + 0.1f * level3 &&
                            VL_Utility.LOS_IsValid(item, player.GetCenterPoint(), player.transform.position + player.transform.up * 0.15f))
                        {
                            float fireExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_fire * (1f + (level3 / 100f));
                            float frostExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_frost * (1f + (level3 / 100f));
                            float lightningExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_lightning * (1f + (level3 / 100f));
                            float poisonExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_poison * (1f + (level3 / 100f));
                            float spiritExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_spirit * (1f + (level3 / 100f));

                            UnityEngine.Vector3 dir = item.transform.position - player.transform.position;
                            HitData hitData2 = new HitData();
                            hitData2.m_damage.m_frost = UnityEngine.Random.Range(10f + 0.5f * level3, 20f + level3) * VL_GlobalConfigs.g_DamageModifer * VL_GlobalConfigs.c_mageFrostNova + frostExpertiseDamage;
                            hitData2.m_damage.m_fire = fireExpertiseDamage;
                            hitData2.m_damage.m_lightning = lightningExpertiseDamage;
                            hitData2.m_damage.m_poison = poisonExpertiseDamage;
                            hitData2.m_damage.m_spirit = spiritExpertiseDamage;
        ...
        else if (VL_Utility.Ability1_Input_Down)
        {
            if (!player.GetSEMan().HaveStatusEffect("SE_VL_Ability1_CD".GetStableHashCode()))
            {
                ...
                else if (player.GetStamina() >= VL_Utility.GetFireballCost)
                {
                    ValheimLegends.shouldUseGuardianPower = false;
                    ...
                    UnityEngine.Vector3 target2 = ((!Physics.Raycast(vector3, player.GetLookDir(), out hitInfo2, float.PositiveInfinity, Script_Layermask) || !hitInfo2.collider)
                        ? (position2 + player.GetLookDir() * 1000f)
                        : hitInfo2.point);

                    float fireExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_fire * level4 / 150f;
                    float frostExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_frost * level4 / 150f;
                    float lightningExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_lightning * level4 / 150f;
                    float poisonExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_poison * level4 / 150f;
                    float spiritExpertiseDamage = player.GetCurrentWeapon().GetDamage().m_spirit * level4 / 150f;

                    HitData hitData3 = new HitData();
                    hitData3.m_damage.m_fire = UnityEngine.Random.Range(5f + 1.6f * level4, 10f + 1.8f * level4) * VL_GlobalConfigs.g_DamageModifer * VL_GlobalConfigs.c_mageFireball + fireExpertiseDamage;
                    ...
                    hitData3.m_damage.m_frost = frostExpertiseDamage;
                    hitData3.m_damage.m_lightning = lightningExpertiseDamage;
                    hitData3.m_damage.m_poison = poisonExpertiseDamage;
                    hitData3.m_damage.m_spirit = spiritExpertiseDamage;
